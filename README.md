...
- $\mathcal{D_{\text{real}}} = \\{(s_i^{\theta_i^+}, a_i)\\}_{i=1}^{N} \rightarrow$ real-world trajectory distribution;
- $\mathcal{D_{\text{sim}}}(\theta) = \\{(s_j', a_j')\\}_{\theta,j=1}^{N} \rightarrow$ simulation trajectory distribution parameterized by $\theta \in \\{\theta_1, \theta_2, \ldots, \theta_n\\}$;
