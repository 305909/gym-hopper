...
def test(args, test_env, seed):
    ...
    if args.train:
        model = f'{args.directory}/TRPO-({args.train_env} to {args.test_env}).mdl'
        agent = TRPO.load(model, 
                          env = env, 
                          device = args.device)
    else:
    ...
    if args.render:
        imageio.mimwrite(f'{args.directory}/TRPO-({args.train_env} to {args.test_env})-test.gif', frames, fps = 30)
...
def main():
    ...
    if args.train:
        ...
        for metric, records in zip(('reward', 'length'), (pool['rewards'], pool['lengths'])):
            ...
            if metric == 'reward':
                path = os.path.join(args.directory, f'TRPO-({args.train_env} to {args.test_env})-rewards.npy')
                np.save(path, ys)
            track(metric, xs, ys, sigmas, args, 
                  label = 'TRPO', 
                  filename = f'TRPO-({args.train_env} to {args.test_env})-{metric}')
        ...
        agent.save(f'{args.directory}/TRPO-({args.train_env} to {args.test_env}).mdl')
        print(f'\nmodel checkpoint storage: {args.directory}/TRPO-({args.train_env} to {args.test_env}).mdl\n')
